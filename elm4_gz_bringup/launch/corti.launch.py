...
def generate_launch_description():
    ...
    # Directories
    pkg_elm4_gz_bringup = get_package_share_directory(
        'elm4_gz_bringup')
    pkg_elm4_gz_resource = get_package_share_directory(
        'elm4_gz_resource')
    pkg_elm4_description = get_package_share_directory(
        'elm4_description')
    pkg_elm4_rviz = get_package_share_directory(
        'elm4_rviz')
    ...
    elm4_ros_gz_bridge_launch = PathJoinSubstitution(
        [pkg_elm4_gz_bringup, 'launch', 'ros_gz_bridge.launch.py'])
    rviz_launch = PathJoinSubstitution(
        [pkg_elm4_rviz, 'launch', 'view_robot.launch.py'])
    ...
    elm4_node_yaml_file = LaunchConfiguration('param_file')
    ...
    # ROS GZ bridge
    elm4_ros_gz_bridge = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([elm4_ros_gz_bridge_launch]),
        launch_arguments=[
    ...
    tf_baselink_base_footprint = Node(
        ...
        arguments=[
            '--x', '0.0', '--y', '0.0', '--z', '0.0',
            '--roll', '0.0', '--pitch', '0.0', '--yaw', '0.0',
            '--frame-id', 'elm4/base_footprint', '--child-frame-id', 'baselink'],
        parameters=[{'use_sim_time': LaunchConfiguration('use_sim_time')}],
        )
    ...
    ld.add_action(elm4_ros_gz_bridge)
